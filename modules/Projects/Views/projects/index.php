...
<td><?= htmlspecialchars($row['progress'] ?? '') ?></td>
                        ...
                        <td class="text-end pe-4">                            <button onclick='openEditModal(<?= json_encode($row) ?>)' class="btn btn-sm btn-light text-primary me-1"><i class="fas fa-edit"></i></button>
                            <form method="POST" action="/projects/delete" class="d-inline">
        ...
        <form method="POST" action="/projects">
            <div class="modal-content border-0 shadow-lg">
                <div class="modal-header bg-light border-0">
                    <h5 class="modal-title fw-bold">Add Projects</h5>
...
<div class="modal fade" id="editModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog">
        <form method="POST" action="/projects/update">
            ...
            <div class="modal-content border-0 shadow-lg">
                <div class="modal-header bg-light border-0">
                    <h5 class="modal-title fw-bold">Edit Projects</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body p-4">
                    ...
                    <div class="mb-3">
                        <label class="form-label fw-bold">Progress</label>
                        <input type="number" name="progress" id="edit_progress" class="form-control" required>
                    </div>
                </div>
                <div class="modal-footer border-0 bg-light">
                    <button type="button" class="btn btn-link text-muted text-decoration-none" data-bs-dismiss="modal">Cancel</button>
...
<script>
    function openEditModal(row) {
        ...
        document.getElementById('edit_name').value = row['name'] || '';
            ...
            document.getElementById('edit_progress').value = row['progress'] || '';
