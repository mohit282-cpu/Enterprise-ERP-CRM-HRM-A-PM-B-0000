<div class="row mb-4">
    <div class="col-12 d-flex justify-content-between align-items-center">
        <h4 class="fw-bold tracking-tight mb-0">Chart of Accounts</h4>
...
<td><?= htmlspecialchars($row['balance'] ?? '') ?></td>
                        ...
                        <td class="text-end pe-4">                            <button onclick='openEditModal(<?= json_encode($row) ?>)' class="btn btn-sm btn-light text-primary me-1"><i class="fas fa-edit"></i></button>
                            <form method="POST" action="/accounting/delete" class="d-inline">
        ...
        <form method="POST" action="/accounting">
            <div class="modal-content border-0 shadow-lg">
                <div class="modal-header bg-light border-0">
                    <h5 class="modal-title fw-bold">Add Chart of Accounts</h5>
...
<div class="modal fade" id="editModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog">
        <form method="POST" action="/accounting/update">
            <input type="hidden" name="id" id="edit_id">
            <div class="modal-content border-0 shadow-lg">
                <div class="modal-header bg-light border-0">
                    <h5 class="modal-title fw-bold">Edit Chart of Accounts</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body p-4">
                    ...
                    <div class="mb-3">
                        <label class="form-label fw-bold">Balance</label>
                        <input type="number" name="balance" id="edit_balance" class="form-control" required>
                    </div>
                </div>
                <div class="modal-footer border-0 bg-light">
                    <button type="button" class="btn btn-link text-muted text-decoration-none" data-bs-dismiss="modal">Cancel</button>
...
<script>
    function openEditModal(row) {
        ...
        document.getElementById('edit_name').value = row['name'] || '';
            ...
            document.getElementById('edit_balance').value = row['balance'] || '';
